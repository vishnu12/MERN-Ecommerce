...
exports.createProduct=(req,res)=>{
    ...
    if(err) return res.status(400).json({error:'problem with the image'})
...
exports.updateStock=(req,res,next)=>{
    ...
    let myOpeartions=req.body.order.products
    .map(product=>{
        return {
            updateOne:{
    ...
    Product.bulkWrite(myOpeartions,{},(err,prod)=>{
        if(err) return res.status(400).json({error:'Bulk operations failed'})
